...
class EmbeddingStage(nn.Module):
    ...
    def forward(self, idx):
        ...
        return self.drop(tok_emb + row_emb + col_emb + chan_emb)
...
# Pack everything into list of LayerSpec objects
def build_gpt_pipeline(config):
    ...
    for _ in range(config.n_layer):
        layers.append(LayerSpec(TransformerStage, config))

    # last stage: final norm + lm head
    layers.append(LayerSpec(FinalStage, config))

    pipeline = PipelineModule(
        layers=layers,
        num_stages=config.pipeline_parallel_stages,
        loss_fn=None,                  # we let FinalStage compute the loss
